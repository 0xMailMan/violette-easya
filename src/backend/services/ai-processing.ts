...
class AIProcessingService implements AIProcessingPipeline {
  ...
  private async analyzePhoto(photoBase64: string): Promise<Partial<ContentAnalysis>> {
    try {
...
Format your response as JSON with the following structure:
{
  ...
  private async analyzeText(text: string): Promise<Partial<ContentAnalysis>> {
    try {
...
Format as JSON with this structure:
{
  ...
  private async analyzeLocationContext(location: LocationData): Promise<string> {
    try {
...
Keep it poetic and meaningful, focusing on the human experience of being in such a place.`;
  ...
  private async refineAnalysis(analysis: ContentAnalysis): Promise<ContentAnalysis> {
    try {
...
Return as JSON with the same structure as the input, but refined and improved.`;
      ...
      const refinedAnalysis = JSON.parse((response.content[0] as any).text);
  ...
  async generateEmbedding(description: string): Promise<number[]> {
    try {
...
Return only 10 decimal numbers separated by commas, no other text.`;
      ...
      const features = (response.content[0] as any).text.split(',').map((f: string) => parseFloat(f.trim()));
      
      // Expand to a larger vector by creating derived features
      const embedding: number[] = [];
      
      // Original features
      features.forEach((f: number) => embedding.push(isNaN(f) ? 0.5 : f));
  ...
  async extractThemes(description: string): Promise<ThemeAnalysis> {
    try {
...
Return as JSON with this exact structure:
{
  ...
  "people": ["person1", "person2", ...]
      ...
      const themes = JSON.parse((response.content[0] as any).text);
  ...
  async sanitizeContent(description: string): Promise<string> {
    try {
...
Return only the sanitized text, maintaining the original tone and style.`;
  ...
  async batchProcess(inputs: Array<{
    ...
    const results = await Promise.allSettled(
      inputs.map(input => this.analyzeContent(input))
    );
  ...
  async validateImageContent(imageBase64: string): Promise<boolean> {
    try {
      // Basic validation - check if it's a valid base64 image
      const buffer = Buffer.from(imageBase64, 'base64');
      return buffer.length > 0 && buffer.length < config.storage.maxFileSize;
    } catch (error) {
      return false;
    }
  }
